...
case class LikesAndDates(likes_count: Long, taken_as_timestamp: Long)
class ExtractLikesByDateSpec extends AnyFlatSpec with Matchers with GivenWhenThen{
  ...
  val expectedData = Seq(LikesAndDates(983475,1619021998),LikesAndDates(283475,1619021990))
  val expectedResult = expectedData.toDF

  "ExtractLikesByDate" should "Extract likes and dates data from input data" in {
    Given("The input data")
    val postsData = ExtractPosts(initialData)

    When("ExtractLikesByDate is invoked")
    val LikesByDates = ExtractLikesWithDate(spark, postsData)

    Then("the extracted Data should be returned")
    LikesByDates.collect() should contain theSameElementsAs expectedResult.collect()
  }
}
